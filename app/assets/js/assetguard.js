        ...
        switch (process.platform) {
            ...
            default:
                ...
                return Promise.resolve({
                    error: `「${process.platform}」は対応していないOSです。<br>Windows(x64)、macOS(x64/M1)のみ対応しています。<br>Linuxは対応していませんのでUzushikaLauncherの使用を諦め、ご自身でModパック導入方法を調べて導入してください。`
                })
        }

        const url = `https://github.com/UzushikaMinecraft/files/releases/download/openjdk/jdk${major}-${sanitizedOS}-${architecture}.${ext}`
        ...
        return new Promise((resolve, reject) => {
            request.head({ url, json: true }, (err, resp) => {
                if (!err && resp.statusCode === 200) {
                    ...
                    resolve({
                        error: `ダウンロードできませんでした。<br>「jdk${major}-${sanitizedOS}-${architecture}.${ext}」が <a href="https://github.com/UzushikaMinecraft/files/releases/tag/openjdk">UzushikaMinecraft/filesのGitHubページ内</a> にあるか確認してください。`
                    })
                }
            })
        })
